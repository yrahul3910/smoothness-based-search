...
class Predictor(Protocol):
    """Protocol class for a generic ML predictor."""
...
def get_smoothness_mle_approx(data: Data, model: Predictor) -> float:
    """Perform a Monte Carlo approximation of the smoothness."""
    rng = np.random.default_rng()

    train_size = len(data.x_train)
    subset_size = max(100, train_size // 10)
    subset_idx = rng.choice(range(train_size), subset_size)
    subset = data.x_train[subset_idx, :]

    eps = 1e-6
    # Generate random directions for all points
    directions = rng.normal(0, 1, subset.shape)
    ...
    preds = model.predict(batch_points)
    ...
    return float(np.mean(cur_smoothness))
